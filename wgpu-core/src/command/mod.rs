...
/// The current state of a [`CommandEncoder`].
#[derive(Debug)]
pub(crate) enum CommandEncoderStatus {
    ...
    /// and `command_encoder_run_compute_pass` require the encoder to
    /// be in this state.
    ///
...
///
/// Each wgpu-core [`CommandBuffer`] owns an instance of this type. This is
/// where the commands are actually kept.
///
...
/// commands into the middle of a recorded stream. However, queue submission
/// accepts a series of command buffers at once, so we can simply break the
/// stream up into multiple buffers, and reorder the buffers. See
/// [`CommandEncoder::close_and_swap`] for a more specific example of this.
///
...
pub(crate) struct CommandEncoder<A: HalApi> {
    /// The underlying `wgpu_hal` command encoder.
    ///
    /// This implements the [`wgpu_hal::CommandEncoder`] trait.
    ///
    ...
    /// [`raw.reset_all()`][CE::ra], so we need to keep them around.
    ///
...
impl<A: HalApi> CommandEncoder<A> {
    ...
    /// and [`transition_textures`], to put the resources the pass
    /// operates on in the appropriate state. Unfortunately, we don't
    /// know which transitions are needed until we're done recording
    /// the pass itself. Rather than iterating over the pass twice, we
    /// note the necessary transitions as we record its commands, and
    /// then jam in a new raw command buffer for the transitions just
    /// before the pass's. This is the function that jams the
    /// transitions in.
    ///
...
impl<A: HalApi> CommandBuffer<A> {
    ...
    ///
    /// The encoder must not yet have been "finished".
    fn get_encoder(
        hub: &Hub<A>,
